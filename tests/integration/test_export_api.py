...
async def _analyzed_project(client):
...
async def test_export_and_download_round_trip(client, fmt):
    project_id, _draft_id = await _analyzed_project(client)
...
async def test_docx_export_reports_inserted_count(client):
    project_id, _draft_id = await _analyzed_project(client)
...
async def test_docx_export_includes_audit_table_when_requested(client, tmp_path):
    project_id, _draft_id = await _analyzed_project(client)
...
async def test_docx_export_excludes_audit_table_when_not_requested(client, tmp_path):
    project_id, _draft_id = await _analyzed_project(client)
    resp = await client.post(
        f"/api/v1/projects/{project_id}/exports",
...
async def test_export_defaults_to_latest_draft_when_unspecified(client):
    project_id, draft_id = await _analyzed_project(client)
    resp = await client.post(f"/api/v1/projects/{project_id}/exports", json={"format": "json"})
    assert resp.status_code == 201
    assert resp.json()["draft_id"] == draft_id


async def test_export_with_no_draft_returns_422(client):
    project_id = await create_project(client, "Empty")
    resp = await client.post(f"/api/v1/projects/{project_id}/exports", json={"format": "json"})
    assert resp.status_code == 422


async def test_list_exports_ordered_newest_first(client):
    project_id, _draft_id = await _analyzed_project(client)
...
async def test_download_unknown_export_404s(client):
    import uuid
...
async def test_insertion_mode_direct_produces_no_tracked_changes(client, tmp_path):
    project_id, _draft_id = await _analyzed_project(client)
    resp = await client.post(
        f"/api/v1/projects/{project_id}/exports",
