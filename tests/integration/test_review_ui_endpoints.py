...
async def test_batch_recommendations_covers_every_needs_citation_claim(client):
...
async def test_batch_recommendations_respects_limit_per_claim(client):
    _project_id, draft_id = await _analyzed_project(client)
    batch = (
...
async def test_decision_endpoint_accepts(client):
    project_id, draft_id = await _analyzed_project(client)
    claims = (await client.get(f"/api/v1/drafts/{draft_id}/claims?needs_citation=true")).json()
...
async def test_decision_endpoint_marks_irrelevant(client):
    project_id, draft_id = await _analyzed_project(client)
    claims = (await client.get(f"/api/v1/drafts/{draft_id}/claims?needs_citation=true")).json()
    recs = (await client.get(f"/api/v1/claims/{claims[0]['id']}/recommendations")).json()
...
async def test_decision_endpoint_rejects_invalid_value(client):
    project_id, draft_id = await _analyzed_project(client)
    claims = (await client.get(f"/api/v1/drafts/{draft_id}/claims?needs_citation=true")).json()
    recs = (await client.get(f"/api/v1/claims/{claims[0]['id']}/recommendations")).json()
...
async def test_irrelevant_removes_a_prior_acceptance(client):
    project_id, draft_id = await _analyzed_project(client)
    claims = (await client.get(f"/api/v1/drafts/{draft_id}/claims?needs_citation=true")).json()
    recs = (await client.get(f"/api/v1/claims/{claims[0]['id']}/recommendations")).json()
    rec_id = recs[0]["id"]
...
async def test_accepted_citations_endpoint_returns_joined_text(client):
    project_id, draft_id = await _analyzed_project(client)
    claims = (await client.get(f"/api/v1/drafts/{draft_id}/claims?needs_citation=true")).json()
...
async def test_project_summary_before_any_upload(client):
...
async def test_project_summary_reflects_analysis_and_acceptance(client):
    project_id, draft_id = await _analyzed_project(client)
    claims = (await client.get(f"/api/v1/drafts/{draft_id}/claims?needs_citation=true")).json()
